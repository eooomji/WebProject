      ...
      if(checkTrue.data) {
        for (let i = 0; i < monthDay.data.length; i++) {
          if(response.data[i] === undefined) response.data[i] = {content : ""};
            ...
            missionHTML += `<div class="missions${i+1} oneDateSel"><input class="form-check-input" type="checkbox" name="${nowMonth+1}" onclick='getCheckedCnt(${i+1}, ${nowMonth+1})' checked />${response.data[i].missionName}</div>`;
            if(j < checkTrueNum - 1) j++;
          }
          else {
            missionHTML += `<div class="missions${i+1} oneDateSel"><input class="form-check-input" type="checkbox" name="${nowMonth+1}" onclick='getCheckedCnt(${i+1}, ${nowMonth+1})'>${response.data[i].missionName}</div>`;       
          }
        }
      }
      else {
        for (let i = 0; i < monthDay.data.length; i++) {
          if(response.data[i] === undefined) response.data[i] = {content : ""};
          else missionHTML += `<div class="missions${i+1} oneDateSel"><input class="form-check-input" type="checkbox" name="${nowMonth+1}" onclick='getCheckedCnt(${i+1}, ${nowMonth+1})'>${response.data[i].missionName}</div>`;
        }
      }
    } 
    else {
      if (`${nowMonth+1} > 12`) {
        const m = parseInt((nowMonth+1)/12);
        alert(`${m}월의 미션 데이터가 없어 로딩 실패`)
      }
      else alert(`${nowMonth+1}월의 미션 데이터가 없어 로딩 실패`);
    }
    j = 0;
