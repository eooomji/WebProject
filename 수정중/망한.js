...
const arr = async(nowMonth, maxDay) => {
  try {
    let missionHTML = '';
    // let j = 0;
    const responsemission = await axios.post('../php/getMissions.php', {userName: userName, nowMonth: nowMonth+1}); // nowMonth+1 -> nowMonth로 수정
    const checkTrue = await axios.post('../php/getCheck.php', {userName: userName, nowMonth: nowMonth+1}); // nowMonth+1 -> nowMonth로 수정
 
    if(responsemission.data) {
      // console.log(responsemission.data[1]);
      // console.log(checkTrue.data[0][`DAY(date)`]);
      if(checkTrue.data) {
      ...
      alert(`${nowMonth}월의 미션 데이터가 없어 로딩 실패`); // 건드림
    }
...
const CheckedCnt = async(month) => {

  const checkTrue = await axios.post('../php/getCheck.php', {month: month+1, userName: userName});
